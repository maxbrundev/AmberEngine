...
AmberGame::Core::Game::Game(Context& p_context) :
	m_context(p_context),
	m_gameRenderer(p_context),
	m_fpsCounter(*p_context.window)
	#ifdef _DEBUG
	,
	m_driverInfo(*p_context.driver, *p_context.window),
	m_gameProfiler(*p_context.window, 0.25f),
	m_frameInfo(*p_context.renderer, *p_context.window)
	#endif
{
	...
	m_context.sceneManager.LoadScene(m_context.projectSettings.GetOrDefault<std::string>("start_scene", "Scenes\\Main.amscene"));
	m_context.sceneManager.GetCurrentScene()->Play();
}

AmberGame::Core::Game::~Game()
{
	m_context.sceneManager.UnloadCurrentScene();
}

void AmberGame::Core::Game::PreUpdate()
{
	#ifdef _DEBUG
...
void AmberGame::Core::Game::Update(float p_deltaTime)
{
	m_elapsed += p_deltaTime;

	m_context.engineUBO->SetSubData(m_elapsed, 3 * sizeof(glm::mat4) + sizeof(glm::vec3));
	...
	if (m_context.inputManager->IsKeyPressed(AmberWindowing::Inputs::EKey::KEY_F12))
		m_showDebugInformation = !m_showDebugInformation;

	#ifdef _DEBUG
	if (m_context.inputManager->IsKeyPressed(AmberWindowing::Inputs::EKey::KEY_R))
		AmberRendering::Resources::Loaders::ShaderLoader::Recompile(*m_context.shaderManager[":Shaders\\Standard.glsl"], "Data\\Engine\\Shaders\\Standard.glsl");
	#endif
...
void AmberGame::Core::Game::PostUpdate()
{
	#ifdef _DEBUG
