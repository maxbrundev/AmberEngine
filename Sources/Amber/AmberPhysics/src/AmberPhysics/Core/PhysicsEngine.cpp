...
std::map<std::pair<AmberPhysics::Entities::PhysicalObject*, AmberPhysics::Entities::PhysicalObject*>, bool> AmberPhysics::Core::PhysicsEngine::m_collisionEvents;

AmberPhysics::Core::PhysicsEngine::PhysicsEngine(const Settings::PhysicsSettings & p_settings)
{
	...
	m_world->setGravity(AmberPhysics::Tools::Conversion::ToBtVector3(p_settings.gravity));

	ListenToPhysicalObjects();
	SetCollisionCallback();
}

void AmberPhysics::Core::PhysicsEngine::PreUpdate()
{
...
void AmberPhysics::Core::PhysicsEngine::PostUpdate()
{
...
bool AmberPhysics::Core::PhysicsEngine::Update(float p_deltaTime)
{
	PreUpdate();
...
std::optional<AmberPhysics::Entities::RaycastHit> AmberPhysics::Core::PhysicsEngine::Raycast(glm::vec3 p_origin, glm::vec3 p_direction, float p_distance)
{
	if (p_direction == glm::vec3(0.0f))
		return {};

	btVector3 origin = AmberPhysics::Tools::Conversion::ToBtVector3(p_origin);
	btVector3 target = AmberPhysics::Tools::Conversion::ToBtVector3(p_origin + p_direction * p_distance);
	...
	if (ClosestRayCallback.hasHit())
	{
		...
		// Get First Hit
		resultHit.FirstResultObject = reinterpret_cast<AmberPhysics::Entities::PhysicalObject*>(ClosestRayCallback.m_collisionObject->getUserPointer());
		...
		for (int i = 0; i < rayCallback.m_collisionObjects.size(); i++)
			resultHit.ResultObjects.push_back(reinterpret_cast<AmberPhysics::Entities::PhysicalObject*>(rayCallback.m_collisionObjects[i]->getUserPointer()));
...
void AmberPhysics::Core::PhysicsEngine::SetGravity(const glm::vec3 & p_gravity)
{
	m_world->setGravity(AmberPhysics::Tools::Conversion::ToBtVector3(p_gravity));
}

glm::vec3 AmberPhysics::Core::PhysicsEngine::GetGravity() const
{
	return AmberPhysics::Tools::Conversion::ToOvVector3(m_world->getGravity());
}

void AmberPhysics::Core::PhysicsEngine::ListenToPhysicalObjects()
{
...
void AmberPhysics::Core::PhysicsEngine::Consider(Entities::PhysicalObject& p_toConsider)
{
	m_physicalObjects.push_back(std::ref(p_toConsider));
}

void AmberPhysics::Core::PhysicsEngine::Unconsider(Entities::PhysicalObject& p_toUnconsider)
{
	{
...
void AmberPhysics::Core::PhysicsEngine::Consider(btRigidBody& p_toConsider)
{
	m_world->addRigidBody(&p_toConsider);
}

void AmberPhysics::Core::PhysicsEngine::Unconsider(btRigidBody& p_toUnconsider)
{
	m_world->removeRigidBody(&p_toUnconsider);
}

void AmberPhysics::Core::PhysicsEngine::ResetCollisionEvents()
{
...
void AmberPhysics::Core::PhysicsEngine::CheckCollisionStopEvents()
{
...
bool AmberPhysics::Core::PhysicsEngine::CollisionCallback(btManifoldPoint& cp, const btCollisionObjectWrapper* obj1, int id1, int index1, const btCollisionObjectWrapper* obj2, int id2, int index2)
{
...
void AmberPhysics::Core::PhysicsEngine::SetCollisionCallback()
{
