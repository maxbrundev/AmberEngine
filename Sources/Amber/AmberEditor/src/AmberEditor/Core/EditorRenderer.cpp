...
AmberEditor::Core::EditorRenderer::EditorRenderer(Context& p_context) : m_context(p_context)
{
	m_context.driver->SetCapability(AmberRendering::Settings::ERenderingCapability::STENCIL_TEST, true);
	m_context.driver->SetStencilOperations(AmberRendering::Settings::EOperation::KEEP, AmberRendering::Settings::EOperation::KEEP, AmberRendering::Settings::EOperation::REPLACE);
	m_context.driver->SetStencilAlgorithm(AmberRendering::Settings::EComparisonAlgorithm::ALWAYS, 1, 0xFF);
...
void AmberEditor::Core::EditorRenderer::InitMaterials()
{
	...
	m_defaultMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", nullptr);
	...
	m_textureMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", nullptr);

	m_outlineMaterial.SetShader(m_context.shaderManager[":Shaders\\Unlit.glsl"]);
	m_outlineMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", nullptr);
	...
	m_stencilFillMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", nullptr);
	...
	m_actorPickingMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", nullptr);
...
void AmberEditor::Core::EditorRenderer::PreparePickingMaterial(AmberCore::ECS::Actor& p_actor, AmberCore::Resources::Material& p_material)
{
...
void AmberEditor::Core::EditorRenderer::RenderSceneForActorPicking()
{
	...
	for (auto modelRenderer : scene.GetFastAccessComponents().modelRenderers)
	{
		...
		if (actor.IsActive())
		{
			...
			{
				if (auto materialRenderer = modelRenderer->Owner.GetComponent<AmberCore::ECS::Components::CMaterialRenderer>())
				{
					const AmberCore::ECS::Components::CMaterialRenderer::MaterialList& materials = materialRenderer->GetMaterials();
...
void AmberEditor::Core::EditorRenderer::RenderUI() const
{
	m_context.uiManager->Render();
}

void AmberEditor::Core::EditorRenderer::RenderCameras()
{
	for (auto camera : m_context.sceneManager.GetCurrentScene()->GetFastAccessComponents().cameras)
	{
		auto& actor = camera->Owner;
...
void AmberEditor::Core::EditorRenderer::RenderGrid(const glm::vec3& p_viewPos, const glm::vec3& p_color, float p_div, float p_bias, float p_LineWidth, float p_MajorLineWidth)
{
...
void AmberEditor::Core::EditorRenderer::UpdateLights(AmberCore::SceneSystem::Scene& p_scene) const
{
...
void AmberEditor::Core::EditorRenderer::RenderCameraPerspectiveFrustum(std::pair<uint16_t, uint16_t>& p_size,
	AmberCore::ECS::Components::CCamera& p_camera)
{
...
void AmberEditor::Core::EditorRenderer::RenderCameraOrthographicFrustum(std::pair<uint16_t, uint16_t>& p_size,
	AmberCore::ECS::Components::CCamera& p_camera)
{
...
void AmberEditor::Core::EditorRenderer::RenderCameraFrustum(AmberCore::ECS::Components::CCamera& p_camera)
{
	...
	{
	case AmberRendering::Settings::EProjectionMode::ORTHOGRAPHIC:
		RenderCameraOrthographicFrustum(gameViewSize, p_camera);
		break;
	
	case AmberRendering::Settings::EProjectionMode::PERSPECTIVE:
		RenderCameraPerspectiveFrustum(gameViewSize, p_camera);
		break;
	}
}

void AmberEditor::Core::EditorRenderer::RenderAmbientBoxVolume(AmberCore::ECS::Components::CAmbientBoxLight& p_ambientBoxLight)
{
	bool depthTestBackup = m_context.renderer->GetDriver().GetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST);
	m_context.renderer->GetDriver().SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, false);
	...
	m_context.renderer->GetDriver().SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, depthTestBackup);
}

void AmberEditor::Core::EditorRenderer::RenderAmbientSphereVolume(
	AmberCore::ECS::Components::CAmbientSphereLight& p_ambientSphereLight)
{
	bool depthTestBackup = m_context.driver->GetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST);
	m_context.driver->SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, false);
	...
	m_context.driver->SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, depthTestBackup);
}

glm::mat4 AmberEditor::Core::EditorRenderer::CalculateCameraModelMatrix(AmberCore::ECS::Actor& p_actor)
{
...
void AmberEditor::Core::EditorRenderer::RenderModelAsset(AmberRendering::Resources::Model& p_model)
{
...
void AmberEditor::Core::EditorRenderer::RenderTextureAsset(AmberRendering::Resources::Texture& p_texture)
{
	auto model = glm::rotate(glm::scale(glm::mat4(1.0f), { 5.f, 5.f, 5.f }), 90.f * 0.0174f, glm::vec3(1.0f, 0.0f, 0.0f));

	m_textureMaterial.SetUniform<AmberRendering::Resources::Texture*>("u_DiffuseMap", &p_texture);
	m_context.renderer->DrawModelWithSingleMaterial(*m_context.editorResources->GetModel("Plane"), m_textureMaterial, &model);
}

void AmberEditor::Core::EditorRenderer::RenderMaterialAsset(AmberCore::Resources::Material& p_material)
{
	auto model = glm::scale(glm::mat4(1.0f) , { 3.f, 3.f, 3.f });
...
void AmberEditor::Core::EditorRenderer::RenderModelToStencil(const glm::mat4& p_worldMatrix, AmberRendering::Resources::Model& p_model)
{
                                                           ...
                                                           float p_width)
{
	...
	m_context.renderer->GetDriver().SetRasterizationMode(AmberRendering::Settings::ERasterizationMode::FILL);
	m_context.renderer->GetDriver().SetStencilAlgorithm(AmberRendering::Settings::EComparisonAlgorithm::ALWAYS, 1, 0xFF);
}

void AmberEditor::Core::EditorRenderer::RenderActorOutlinePass(AmberCore::ECS::Actor& p_actor, bool p_toStencil, bool p_isSelected)
{
	...
	if (p_actor.IsActive())
	{
		...
		if (auto modelRenderer = p_actor.GetComponent<AmberCore::ECS::Components::CModelRenderer>(); modelRenderer && modelRenderer->GetModel())
		{
			if (p_toStencil)
		...
		if (auto cameraComponent = p_actor.GetComponent<AmberCore::ECS::Components::CCamera>(); cameraComponent)
		{
		...
		if (p_isSelected && !p_toStencil)
		{
			if (p_actor.GetComponent<AmberCore::ECS::Components::CPhysicalObject>())
			{
				RenderActorCollider(p_actor);
			}

			if (auto ambientBoxComp = p_actor.GetComponent<AmberCore::ECS::Components::CAmbientBoxLight>())
			{
				RenderAmbientBoxVolume(*ambientBoxComp);
			}

			if (auto ambientSphereComp = p_actor.GetComponent<AmberCore::ECS::Components::CAmbientSphereLight>())
			{
			...
			{
				if (auto light = p_actor.GetComponent<AmberCore::ECS::Components::CLight>())
				{
...
void AmberEditor::Core::EditorRenderer::RenderActorOutlinePass(const std::vector<AmberCore::ECS::Actor*>& p_actor, bool p_toStencil, bool p_isSelected)
{
	float outlineWidth = p_isSelected ? 5.0f : 2.5f;
	...
	for (auto actor : p_actor)
	{
		...
		{
			/* Render static mesh outline and bounding spheres */
			if (auto modelRenderer = actor->GetComponent<AmberCore::ECS::Components::CModelRenderer>(); modelRenderer && modelRenderer->GetModel())
			{
				if (p_toStencil)
			...
			/* Render camera component outline */
			if (auto cameraComponent = actor->GetComponent<AmberCore::ECS::Components::CCamera>(); cameraComponent)
			{
			...
			if (p_isSelected && !p_toStencil)
			{
				if (actor->GetComponent<AmberCore::ECS::Components::CPhysicalObject>())
				{
					RenderActorCollider(*actor);
				}

				if (auto ambientBoxComp = actor->GetComponent<AmberCore::ECS::Components::CAmbientBoxLight>())
				{
					RenderAmbientBoxVolume(*ambientBoxComp);
				}

				if (auto ambientSphereComp = actor->GetComponent<AmberCore::ECS::Components::CAmbientSphereLight>())
				{
					RenderAmbientSphereVolume(*ambientSphereComp);
				}
				...
				if (Settings::EditorSettings::ShowLightBounds)
				{
					if (auto light = actor->GetComponent<AmberCore::ECS::Components::CLight>())
					{
						RenderLightBounds(*light);
					}
				}
...
void AmberEditor::Core::EditorRenderer::RenderActorCollider(AmberCore::ECS::Actor& p_actor)
{
	bool depthTestBackup = m_context.renderer->GetDriver().GetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST);
	m_context.renderer->GetDriver().SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, false);

	/* Draw the box collider if any */
	if (auto boxColliderComponent = p_actor.GetComponent<AmberCore::ECS::Components::CPhysicalBox>(); boxColliderComponent)
	{
	...
	if (auto sphereColliderComponent = p_actor.GetComponent<AmberCore::ECS::Components::CPhysicalSphere>(); sphereColliderComponent)
	{
		glm::vec3 actorScale = p_actor.transform.GetWorldScale();
		glm::quat rotation = p_actor.transform.GetWorldRotation();
		glm::vec3 center = p_actor.transform.GetWorldPosition();
	...
	if (auto capsuleColliderComponent = p_actor.GetComponent<AmberCore::ECS::Components::CPhysicalCapsule>(); capsuleColliderComponent)
	{
...
void AmberEditor::Core::EditorRenderer::RenderBoundingSpheres(AmberCore::ECS::Components::CModelRenderer& p_modelRenderer)
{
	bool depthTestBackup = m_context.renderer->GetDriver().GetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST);
	m_context.renderer->GetDriver().SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, false);
	...
	if (auto model = p_modelRenderer.GetModel())
	{
		...
		const auto& modelBoundingSphere = model->GetBoundingSphere();
		...
		glm::vec3 sphereOffset = actorRotation * modelBoundingSphere.Position * radiusScale;
...
void AmberEditor::Core::EditorRenderer::RenderLightBounds(AmberCore::ECS::Components::CLight& p_light)
{
	bool depthTestBackup = m_context.renderer->GetDriver().GetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST);
	m_context.renderer->GetDriver().SetCapability(AmberRendering::Settings::ERenderingCapability::DEPTH_TEST, false);
...
void AmberEditor::Core::EditorRenderer::RenderGizmo(const glm::vec3& p_position, const glm::quat& p_rotation, Core::EGizmoOperation p_operation, bool p_pickable, int p_highlightedAxis)
{
	glm::mat4 model = glm::translate(glm::mat4(1.0f), p_position) * glm::mat4_cast(glm::normalize(p_rotation));

	AmberRendering::Resources::Model* arrowModel = nullptr;
