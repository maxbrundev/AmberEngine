...
{
	struct API_AMBERENGINE ObjIndexData
	{
		unsigned int vertexIndex;
		unsigned int uvIndex;
		unsigned int normalIndex;
		bool operator<(const ObjIndexData& p_index) const { return vertexIndex < p_index.vertexIndex; }
	};

	struct API_AMBERENGINE ObjModel
	{
	...
	{
	private:
		std::vector<ObjIndexData> m_indices;
	...
	private:
		...
		void CalculateNormals(ObjModel& p_model);
		ObjIndexData ParseObjIndex(const std::string& p_source);
		...
		unsigned int FindLastVertexIndex(const std::vector<ObjIndexData*>& p_indexLookup, const ObjIndexData* p_currentIndex, const ObjModel& p_result);
