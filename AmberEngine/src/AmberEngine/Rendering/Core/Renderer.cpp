...
void AmberEngine::Core::Renderer::Clear(Rendering::Entities::Camera& p_camera, bool p_colorBuffer, bool p_depthBuffer, bool p_stencilBuffer) const
{
...
uint8_t AmberEngine::Core::Renderer::FetchGLState() const
{
	uint8_t result = 0;
	...
	if (m_driver.GetCapability(Rendering::Settings::ERenderingCapability::BLEND))			result |= 0b0000'0100;
	if (m_driver.GetCapability(Rendering::Settings::ERenderingCapability::CULL_FACE))		result |= 0b0000'1000;
	if (m_driver.GetCapability(Rendering::Settings::ERenderingCapability::DEPTH_TEST))	result |= 0b0001'0000;

	switch (static_cast<Rendering::Settings::ECullFace>(m_driver.GetInt(GL_CULL_FACE)))
	{
...
void AmberEngine::Core::Renderer::ApplyStateMask(uint8_t p_mask)
{
	if (p_mask != m_state)
	{
...
void AmberEngine::Core::Renderer::SetState(uint8_t p_state)
{
	m_state = p_state;
}
...
void AmberEngine::Core::Renderer::Draw(Resources::IMesh& p_mesh, Rendering::Settings::EPrimitiveMode p_primitiveMode, uint32_t p_instances)
{
	if (p_instances > 0)
	{
